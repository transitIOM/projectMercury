...
type ReportManager interface {
	CreateIssueFromReport(ctx context.Context, title string, description string, email string, tags []string) error
}
...
// @Router       /report/ [post]
func PostReport(rm ReportManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := rm.CreateIssueFromReport(ctx, props.Title, props.Description, props.Email, tags)
		if err != nil {
			log.Error(err)
